...
if ( ! class_exists( 'DLM_Gutenberg' ) ) {
	/**
	 * DLM_Gutenberg class.
	 *
	 * Manages the Gutenberg blocks.
	 */
	class DLM_Gutenberg {

		/**
		 * Setup class actions and hooks
		 */
		...
		public function load() {
			...
			$asset_file = include plugin_dir_path( DLM_PLUGIN_FILE )
			                      . 'assets/blocks/dist/blocks.asset.php';

			// register Gutenberg JS
			wp_register_script(
				'dlm_gutenberg_blocks',
				plugins_url( '/assets/blocks/dist/blocks.js',
					download_monitor()->get_plugin_file() ),
			...
			wp_register_style(
				...
				plugins_url( '/assets/css/gb-editor.min.css',
					download_monitor()->get_plugin_file() ),
			...
			wp_set_script_translations( 'dlm_gutenberg_blocks',
				'download-monitor',
				plugin_dir_path( DLM_PLUGIN_FILE ) . 'languages' );
			...
			foreach (
				download_monitor()->service( 'template_handler' )
				                  ->get_available_templates() as $template_key
			=> $template_value
			) {
				$templates[] = array(
					'value' => $template_key,
		...
		public function render_download_button( $attributes, $content ) {
			...
			if ( ! isset( $attributes['download_id'] ) ) {
				...
				if ( current_user_can( 'manage_options' ) ) {
					...
					return esc_html__( 'Please select a download id',
						'download-monitor' );
				} else {
			...
			try {
				...
				$download = download_monitor()->service( 'download_repository' )
				                              ->retrieve_single( absint( $attributes['download_id'] ) );
				...
				if ( isset( $attributes['version_id'] ) ) {
					...
					try {
						...
						$version = download_monitor()
							...
							->retrieve_single( absint( $attributes['version_id'] ) );
			...
			ob_start();
			if ( $download ) {
				$template_handler->get_template_part( 'content-download',
					$template, '', array(
						'dlm_download'   => $download,
						'dlm_attributes' => $attributes,
					) );
			}
			$output = ob_get_clean();

			if ( isset( $attributes['autop'] )
			     && $attributes['autop'] == "1"
			) {
