...
namespace Never5\DownloadMonitor\Shop\Dependencies\PayPal\Core;

use Never5\DownloadMonitor\Shop\Dependencies\PayPal\Log\PayPalLogFactory;
...
class PayPalLoggingManager
{
    ...
    private function __construct($loggerName)
    {
        ...
        $factory = array_key_exists('log.AdapterFactory', $config) && in_array('Never5\DownloadMonitor\Shop\Dependencies\PayPal\Log\PayPalLogFactory', class_implements($config['log.AdapterFactory'])) ? $config['log.AdapterFactory'] : '\Never5\DownloadMonitor\Shop\Dependencies\PayPal\Log\PayPalDefaultLogFactory';
