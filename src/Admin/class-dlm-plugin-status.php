...
class DLM_Plugin_Status {
	...
	public function download_path_backwards_compat( $first_install ){
		...
		if ( is_multisite() && $site_option && '' != $site_option ) {
			...
			$settings = array( 'dlm_downloads_path' => $site_option, 'dlm_crossite_file_browse' => '0', 'dlm_turn_off_file_browser' => '0' );
	...
	public function save_network_downloads_settings(){
		...
		$settings = array( 'dlm_downloads_path' => $downloads_path, 'dlm_crossite_file_browse' => $cross_browse, 'dlm_turn_off_file_browser' => $browse_button );
	...
	public function network_downloads_settings_page(){
		...
		$network_settings = array(
			'dlm_downloads_path' => array(
				'name'     => 'dlm_downloads_path',
				'std'      => '',
				'label'    => __( 'Other downloads path', 'download-monitor' ),
				'desc'     => __( '<strong>!!ATTENTION!! ONLY</strong> modify this setting if you know and are certain of what you are doing. This can cause problems on the download/saving Downloads process if not specified correctly. Prior to modifying this it is advised to <strong>BACKUP YOU DATABASE</strong> in case something goes wrong.<br><br> By default, due to some security issues and restrictions, we only allow downloads from root folder and uploads folder, depending on how your WordPress installation in configured. To be able to download files from somewhere else please specify the path or a more higher path.<br><br>You can use <code>{site_id}</code> placeholder in your path so it will be repalced with site\'s id.<br><br>A full documentation can be seen <a href="https://www.download-monitor.com/kb/add-path/" target="_blank">here</a>.', 'download-monitor' ),
				'type'     => 'text',
			),
		...
		echo '<form method="post" action="settings.php" novalidate="novalidate">';
