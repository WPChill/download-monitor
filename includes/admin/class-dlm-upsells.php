...
class DLM_Upsells {
	...
	public function misc_tab_upsell() {
		...
		if ( ! $this->check_extension( 'dlm-buttons' ) ) {
			...
			echo $this->generate_upsell_box( 'Buttons', 'The Buttons extension allows you to customize your download buttons as you please in order to improve the user experience. Create stunning buttons without needing any coding skills!', 'misc', 'buttons' );
		}
...
', 'endpoint', 'csv-impoter' );
	...
	public function output_amazon_s3_upsell() {
		...
		echo $this->generate_upsell_box( '', esc_html__( 'Use Amazon S3 links for Download Monitor files to run secure, expiring download links.', 'download-monitor' ), 'amazon_s3', 'amazon-s3' );
	...
	public function output_google_drive_upsell() {
		...
		echo $this->generate_upsell_box( '', esc_html__( 'With this extension, you can integrate your files from Google Drive into Download Monitor.', 'download-monitor' ), 'google_drive', 'google-drive' );
