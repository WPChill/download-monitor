...
install_test_suite() {
	...
	sed $ioption "s/wptests_/dlmtests_/" wp-tests-config.php
	sed $ioption "s/example.org/download-monitor.com/" wp-tests-config.php
	sed $ioption "s/admin@example.org/tests@download-monitor.com/" wp-tests-config.php
	sed $ioption "s/Test Blog/Download Monitor Unit Tests/" wp-tests-config.php
}
